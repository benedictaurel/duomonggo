...
import { LogOut, ArrowLeft, Check, X, HelpCircle, Award } from "lucide-react";
...
export default function CoursePage() {
  ...
  useEffect(() => {
    const checkCourseCompletion = async () => {
      ...
      try {
        ...
        setCourseCompleted(data.payload);
      } catch (err) {
  ...
  useEffect(() => {
    const startCourseEnrollment = async () => {
      ...
      try {
        const response = await fetch("http://localhost:8091/enrollments/start", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const renderQuestion = () => {
    ...
    if (isSuccessful) {
      try {
        console.log(`Marking course ${courseId} as complete for user ${accountId}`);
        const response = await fetch("http://localhost:8091/enrollments/complete", {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            "Accept": "application/json"
      ...
      <div className="text-center py-10 space-y-8">
        <div className="flex justify-center">
          <div className="relative inline-block">
            ...
            <span className="mt-5 flex items-center justify-center text-purple-600 font-bold text-xl">
              {percentage}%
            </span>
          </div>
        </div>
            ...
            <div className="px-4 py-3 bg-green-50 border border-green-200 rounded-lg inline-block">
              <div className="flex items-center">
                ...
                <p className="text-green-700 font-medium">
                  You've earned {courseData.expReward} XP!
                </p>
              </div>
            </div>
          ) : (
          ...
          <div className="max-w-3xl mx-auto">
            ...
            {isLoading ? (
              ...
              <div className="bg-purple-50 border border-purple-200 p-6 rounded-lg mb-6">
                <h3 className="font-semibold text-xl text-purple-700 mb-2">You've already completed this course!</h3>
                <p className="text-gray-700 mb-4">You're currently retaking this course. Any new completion will be recorded, but you won't earn additional XP rewards.</p>
                {renderQuestion()}
              </div>
            ) : (
              renderQuestion()
